<template>
  <div class="orders-page" dir="rtl">
    ...
    <div class="page-header">
      ...
      <RouterLink v-if="activeRole === 'client'" to="/services" class="new-order-btn">
        ➕ طلب جديد
      </RouterLink>
    </div>
    ...
    <TransitionGroup v-else name="order-list" tag="div" class="orders-list">
      <div
        ...
        :key="order.id"
        class="order-card"
        :class="{ 'order-card--urgent': isUrgent(order) }"
      >
        <!-- Card header -->
          ...
          <div class="order-card__service">
            ...
            <div>
              ...
              <div class="order-service-meta">
                {{ activeRole === 'client' ? `المستقل: ${order.seller?.name}` : `العميل: ${order.client?.name}` }}
                · <span class="order-id">#{{ order.id }}</span>
              </div>
            </div>
          </div>
          <div class="order-card__badges">
            <span v-if="isUrgent(order)" class="urgent-badge">⚡ عاجل</span>
            <span v-if="order.has_dispute" class="dispute-badge">⚠️ نزاع</span>
        ...
        <div class="order-card__body">
          <div class="order-meta-grid">
            ...
            <div class="meta-item">
              ...
              <span class="meta-value" :class="{ overdue: isOverdue(order.deadline_at) }">
        ...
        <div class="order-card__footer">
          ...
          <div class="order-actions">
            ...
            <button class="action-btn action-btn--ghost" @click="viewOrder(order)">
              🔍 التفاصيل
            </button>
          </div>
        </div>
    ...
    <div v-if="ordersStore.meta.last_page > 1" class="pagination">
      ...
      <span class="page-info">
        صفحة {{ ordersStore.meta.current_page }} من {{ ordersStore.meta.last_page }}
      </span>
      <button
        class="page-btn"
          ...
          <div class="modal">
            ...
            <div class="modal__body">
              ...
              <div class="rating-section">
                ...
                <span class="rating-hint" v-if="completeModal.rating">
                  {{ ratingHints[completeModal.rating - 1] }}
                </span>
              </div>
              <!-- Review text -->
              <div class="field">
                ...
                <textarea
                  ...
                  placeholder="شاركنا تجربتك مع هذا المستقل..."
                  rows="3"
                ></textarea>
              </div>
            </div>
            <div class="modal__footer">
              ...
              <button class="modal-btn modal-btn--success" @click="submitComplete" :disabled="actionLoading">
                <span v-if="actionLoading" class="inline-spinner"></span>
                {{ actionLoading ? 'جاري الإغلاق...' : 'تأكيد القبول وإغلاق الطلب' }}
              </button>
            </div>
          </div>
        </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="disputeModal.open" class="modal-overlay" @mousedown.self="disputeModal.open = false">
          <div class="modal">
            <div class="modal__header">
              <h2 class="modal__title">⚠️ فتح نزاع</h2>
            ...
            <div class="modal__footer">
              ...
              <button class="modal-btn modal-btn--danger" @click="submitDispute" :disabled="actionLoading">
                <span v-if="actionLoading" class="inline-spinner"></span>
                {{ actionLoading ? 'جاري الإرسال...' : 'تقديم النزاع' }}
              </button>
            </div>
          </div>
        </div>
...
const statusTabs = [
  ...
  { key: 'delivered',   icon: '📬', label: 'مُسلَّم' },
...
async function handleCancel(order) {
  ...
  try {
    ...
    showToast('تم إلغاء الطلب وإعادة المبلغ إلى محفظتك ↩️', 'success')
    ordersStore.fetchMyOrders(props.activeRole, '')
  } catch (e) {
    showToast(e.response?.data?.message ?? 'حدث خطأ', 'error')
  } finally { actionLoading.value = null }
...
async function submitDispute() {
  ...
  try {
    ...
    showToast('تم فتح النزاع، ستتواصل معك الإدارة قريباً ⚖️', 'success')
    ordersStore.fetchMyOrders(props.activeRole, '')
  } catch (e) {
    showToast(e.response?.data?.message ?? 'حدث خطأ', 'error')
  } finally { actionLoading.value = null }
...
function isUrgent(order) {
  if (!order.deadline_at || order.status === 'completed' || order.status === 'cancelled') return false
...
function categoryEmoji(cat) {
  const m = { 'تطوير الويب':'💻','تصميم الجرافيك':'🎨','التسويق الرقمي':'📣','كتابة المحتوى':'✍️','الترجمة':'🌐','الفيديو والموشن':'🎬' }
  return m[cat] ?? '⚡'
}
...
onMounted(() => ordersStore.fetchMyOrders(props.activeRole))
watch(() => props.activeRole, r => { activeStatus.value = 'all'; ordersStore.fetchMyOrders(r) })
</script>
...
.new-order-btn {
  ...
  background: var(--grad-primary); color: white; text-decoration: none;
...
.status-tab.active { background: var(--grad-primary); color: white; box-shadow: 0 2px 10px rgba(99,102,241,0.35); }
...
.status--in_progress { background: rgba(99,102,241,0.15); color: var(--color-primary); }
.status--delivered   { background: rgba(139,92,246,0.15); color: var(--color-primary-2); }
.status--completed   { background: rgba(16,185,129,0.15); color: var(--color-success); }
.status--cancelled   { background: rgba(239,68,68,0.12); color: var(--color-error); }
...
.meta-value--price { color: var(--color-primary); font-family: var(--font-display); font-size: 16px; font-weight: 800; }
.meta-value.overdue { color: var(--color-error); }
.order-requirements { font-size: 13px; color: var(--color-text-2); background: var(--color-bg-2); border-radius: var(--radius-md); padding: 10px 14px; line-height: 1.6; }
...
.action-btn--primary { background: var(--grad-primary); color: white; box-shadow: 0 0 16px rgba(99,102,241,0.3); }
.action-btn--success { background: rgba(16,185,129,0.15); color: var(--color-success); border: 1px solid rgba(16,185,129,0.3); }
.action-btn--warning { background: rgba(245,158,11,0.12); color: var(--color-gold); border: 1px solid rgba(245,158,11,0.3); }
.action-btn--danger  { background: rgba(239,68,68,0.1); color: var(--color-error); border: 1px solid rgba(239,68,68,0.25); }
.action-btn--ghost   { background: var(--color-bg-2); color: var(--color-text-2); border: 1px solid var(--color-border); }
...
.field-textarea { width: 100%; padding: 12px 14px; background: var(--color-bg-2); border: 1px solid var(--color-border); border-radius: var(--radius-md); color: var(--color-text); font-family: var(--font-body); font-size: 14px; outline: none; resize: vertical; min-height: 80px; transition: border-color 0.2s; }
.field-textarea:focus { border-color: var(--color-primary); }
.field-error { font-size: 12px; color: var(--color-error); }
.required { color: var(--color-error); }
.dispute-warning { display: flex; gap: 10px; align-items: flex-start; background: rgba(245,158,11,0.08); border: 1px solid rgba(245,158,11,0.25); border-radius: var(--radius-md); padding: 14px 16px; font-size: 14px; color: var(--color-text-2); line-height: 1.7; }
