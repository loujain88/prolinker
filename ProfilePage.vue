    ...
    <Transition name="tab-content" mode="out-in">
      <div v-if="activeTab === 'info'" key="info" class="form-card">
        ...
        <div class="form-footer">
          <button class="save-btn" @click="saveInfo" :disabled="savingInfo">
            <span v-if="savingInfo" class="inline-spinner"></span>
      ...
      <div v-else-if="activeTab === 'security'" key="security" class="form-card">
        ...
        <Transition name="alert">
          <div v-if="securityAlert.msg" class="alert" :class="`alert--${securityAlert.type}`">
            {{ securityAlert.msg }}
          </div>
        </Transition>
        ...
        <div class="form-grid form-grid--narrow">
          <div class="field field--full">
            ...
            <div class="field-input-wrap">
              ...
              <button type="button" class="field-toggle" @click="showCurrent = !showCurrent">{{ showCurrent ? '🙈':'👁️' }}</button>
            </div>
          </div>
          <div class="field">
            ...
            <div class="field-input-wrap">
              <input v-model="pwForm.new" :type="showNew ? 'text' : 'password'"
                class="field-input" placeholder="••••••••" dir="ltr" />
              <button type="button" class="field-toggle" @click="showNew = !showNew">{{ showNew ? '🙈':'👁️' }}</button>
            </div>
        ...
        <div class="form-footer">
          <button class="save-btn" @click="changePassword" :disabled="savingPw">
            <span v-if="savingPw" class="inline-spinner"></span>
      ...
      <div v-else-if="activeTab === 'account'" key="account" class="form-card">
        ...
        <div class="account-info-grid">
          ...
          <div class="ainfo-item" v-if="auth.user?.role === 'seller'">
            ...
            <span class="ainfo-value" :class="auth.user?.seller?.is_verified ? 'ainfo-value--active' : 'ainfo-value--pending'">
              {{ auth.user?.seller?.is_verified ? '✓ موثق' : '⏳ في انتظار التوثيق' }}
            </span>
          </div>
        </div>
        ...
        <div class="danger-zone">
          <h4 class="danger-zone__title">⚠️ منطقة الخطر</h4>
          <p class="danger-zone__desc">حذف الحساب نهائي ولا يمكن التراجع عنه. جميع بياناتك وطلباتك ستُحذف.</p>
          <button class="danger-btn" @click="confirmDeleteAccount">🗑️ حذف الحساب نهائياً</button>
        </div>
      </div>
    </Transition>
...
const tabs = [
  ...
  { key: 'account',  icon: '⚙️', label: 'الحساب' },
]
...
const roleLabel = computed(() =>
  auth.user?.role === 'admin' ? 'مدير المنصة' :
...
const strengthColor = computed(() =>
  ['#EF4444','#F59E0B','#F59E0B','#10B981','#10B981'][Math.max(0, strengthScore.value - 1)] || '#EF4444'
)
...
.avatar-circle  { width: 80px; height: 80px; border-radius: 50%; background: var(--grad-primary); display: flex; align-items: center; justify-content: center; font-size: 28px; font-weight: 800; color: white; box-shadow: 0 0 30px rgba(99,102,241,0.4); }
...
.profile-tab.active { background: var(--grad-primary); color: white; box-shadow: 0 2px 10px rgba(99,102,241,0.35); }
...
/* Save button */
.save-btn { display: inline-flex; align-items: center; gap: 8px; padding: 11px 28px; border-radius: var(--radius-full); background: var(--grad-primary); border: none; color: white; font-family: var(--font-display); font-size: 15px; font-weight: 700; cursor: pointer; box-shadow: 0 0 20px rgba(99,102,241,0.35); transition: opacity 0.2s, transform 0.15s; }
