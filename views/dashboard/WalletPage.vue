<template>
 ...
 <p class="balance-hero__sub"> {{ activeRole === 'client'
 ...
 <div class="tx-row__icon-wrap" :class="`tx-icon--${tx.status}`"> {{ txIcon(tx.status) }}
 </div>
 <div class="tx-row__body">
 <div class="tx-row__title"> {{ activeTab === 'deposits' ? 'طلب شحن رصيد' : 'طلب سحب رصيد' }}
 <span class="tx-id">#{{ tx.id }}</span>
 </div>
 <div class="tx-row__meta"> {{ formatDate(tx.created_at) }}
 ...
 <div class="tx-amount" :class="activeTab === 'deposits' ? 'positive' : 'negative'"> {{ activeTab === 'deposits' ? '+' : '-' }}${{ tx.amount }}
 </div>
 <!-- Receipt link -->
 <button
 ...
 <div class="file-preview__icon"> {{ depositForm.receipt.type === 'application/pdf' ? '' : '' }}
 </div>
 ...
 <!-- Payout image: QR code, Sham Cash, or written delivery info -->
 <div class="field" :class="{ 'field--error': formErrors.payout_image }">
 <label class="field-label">صورة استلام المبلغ (QR شام كاش أو معلومات التسليم)</label>
 <input type="file" class="field-file" accept=".jpg,.jpeg,.png,.pdf" @change="onPayoutImageChange" />
 <span class="field-hint">ارفع صورة واضحة لكود QR أو معلومات التواصل/التسليم حتى تتم مراجعتها من الإدارة</span>
 <span v-if="payoutImageName" class="file-chip"> {{ payoutImageName }}</span>
 <span v-if="formErrors.payout_image" class="field-error">{{ formErrors.payout_image }}</span>
 </div>
...
<script setup> import { ref, computed, watch, onMounted, watchEffect } from 'vue'
...
const displayedList = computed(() => activeTab.value === 'deposits' ? walletStore.deposits : walletStore.withdrawals
)
...
const formErrors = ref({ amount: '', receipt: '', payout_image: '' })
...
const payoutImageFile = ref(null)
const payoutImageName = ref('')
function onPayoutImageChange(e) {
 const file = e.target.files?.[0] ?? null
 payoutImageFile.value = file
 payoutImageName.value = file?.name ?? ''
 formErrors.value.payout_image = ''
}

function openModal(type) {
 modalType.value = type
 formErrors.value = { amount: '', receipt: '', payout_image: '' }
...
function closeModal() {
 ...
 withdrawForm.value = { amount: null, notes: '' }
 payoutImageFile.value = null
 payoutImageName.value = ''
...
async function submitDeposit() {
  ...
  let isSuccess = false // 🟢 متغيرة للتأكد من نجاح العملية
  ...
  try {
    ...
    isSuccess = true // 🟢 تمت العملية بنجاح!
    ...
    // 🟢 1. ننهي حالة الإرسال أولاً حتى يسمح النظام بالإغلاق
    submitting.value = false

    // 🟢 2. إذا نجح الإرسال، نغلق النافذة ونفرغ النموذج
    if (isSuccess) {
      // تفريغ البيانات
      depositForm.value = { amount: '', payment_method: 'bank_transfer', receipt: null }
...
async function submitWithdraw() {
 ...
 if (!payoutImageFile.value) {
 formErrors.value.payout_image = 'الرجاء إرفاق صورة استلام المبلغ'
 return
 }
 ...
 try {
 const fd = new FormData()
 fd.append('amount', withdrawForm.value.amount)
 fd.append('payout_image', payoutImageFile.value)
 fd.append('notes', withdrawForm.value.notes || '')

 await walletStore.submitWithdrawal(fd)
 closeModal()
...
const txIcon = s => ({ approved:'', rejected:'' }[s] ?? '')
const methodLabel = m => ({
 bank_transfer:'تحويل بنكي', cash:'كاش', paypal:'PayPal', wise:'Wise', crypto:'عملة رقمية', qr_photo:'صورة استلام (QR/شام كاش)'
}[m] ?? m)

function formatDate(iso) {
 return new Date(iso).toLocaleDateString('ar-EG-u-nu-latn', { year:'numeric', month:'short', day:'numeric', hour:'2-digit', minute:'2-digit' })
}

onMounted(() => walletStore.fetchSummary())
</script>

<style scoped> .wallet-page { display: flex; flex-direction: column; gap: 24px; }
...
.balance-num { font-family: var(--font-display); font-size: 52px; font-weight: 900; background: white; -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; line-height: 1; }
...
.action-btn--primary { background: rgb(80, 109, 190); color: white; box-shadow: 0 0 24px rgba(99,102,241,0.4); }
.action-btn--primary:hover { box-shadow: 0 0 36px rgba(99,102,241,0.6); }

/* Tabs */
.tab-bar { display: flex; gap: 4px; border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: 4px; width: fit-content; }
.tab-btn {
...
.tab-btn.active { background: rgb(80, 109, 190); color: white; box-shadow: 0 2px 12px rgba(99,102,241,0.35); }
...
.tx-row__meta { font-size: 12px; color: black; margin-top: 3px; }
...
.tx-status--pending { background: rgb(80, 109, 190); color: white; }
...
.tx-amount.positive { color: rgb(48, 155, 15); }
...
.field-prefix { position: absolute; right: 14px; font-size: 15px; color: black; pointer-events: none; z-index: 1; }
.field-input {
...
.field-input--prefixed { padding-right: 36px; background: white; color: black;}
.field-input--prefixed::placeholder {
  color: gray; 
...
.field-file { width: 100%; font-size: 12px; padding: 8px; border: 1px dashed var(--color-border); border-radius: var(--radius-md); background: var(--color-bg-2); color: var(--color-text-2); cursor: pointer; }
.field-hint { display: block; font-size: 11px; color: var(--color-text-3); margin-top: 4px; }
...
.method-opt {
 ...
 border: 1px solid var(--color-border);
 background: rgb(80, 109, 190); cursor: pointer;
...
.modal-btn--primary { background:rgb(80, 109, 190); color: white; border: 1px solid rgba(99,102,241,0.35); }
.modal-btn--primary:hover:not(:disabled) { opacity: 0.9; transform: translateY(-1px); }
.modal-btn--ghost { background: rgb(80, 109, 190); border: 1px solid var(--color-border); color: var(--color-text-2); }
