      ...
      <div v-for="svc in services" :key="svc.id" class="svc-card">
        ...
        <div class="svc-thumb">
          ...
          <div class="svc-thumb__overlay">
            <button class="thumb-action" @click="openModal(svc)" title="تعديل">
            ...
            <button class="thumb-action thumb-action--danger" @click="confirmDelete(svc)" title="حذف">
          ...
          <span class="svc-status-dot" :class="svc.is_pending_review ? 'dot--pending' : `dot--${svc.status}`" :title="displayStatusLabel(svc)"></span>
        </div>
        ...
        <div class="svc-footer">
          ...
          <div class="svc-actions">
            ...
            <span v-else class="pending-pill">قيد مراجعة الإدارة</span>
            <button class="edit-btn" @click="openModal(svc)">تعديل</button>
          </div>
        </div>
      </div>
    </TransitionGroup>
        ...
        <div v-if="modal.open" class="modal-overlay" @mousedown.self="closeModal">
          <div class="modal modal--wide" role="dialog" :aria-label="modal.isEdit ? 'تعديل الخدمة' : 'خدمة جديدة'">
            ...
            <div class="modal__header">
              <h2 class="modal__title">{{ modal.isEdit ? 'تعديل الخدمة' : 'خدمة جديدة' }}</h2>
            ...
            <div class="modal__body">
              ...
              <div class="field">
                ...
                <select v-model="form.category_id" class="field-select" :disabled="categoriesLoading">
                  <option :value="null"> {{ categoriesLoading ? 'جاري تحميل التصنيفات...' : categories.length === 0 ? 'لا توجد تصنيفات بعد' : '— اختر فئة —' }}
                  </option>
                  <option v-for="c in categories" :key="c.id" :value="c.id"> {{ c.icon ?? '' }} {{ c.name }}
                  </option>
                </select>
                <span v-if="!categoriesLoading && categories.length === 0" class="field-hint"> لم يضف الأدمن أي تصنيفات بعد — يمكنك ترك هذا الحقل فارغاً
                </span>
              </div>
              ...
              <!-- Price & Delivery Days Row (جنباً إلى جنب) -->
              <div class="form-row">
                ...
                >
                  ...
                  <Transition name="fade" mode="out-in">
                    <div v-if="!thumbFile && !form.thumbnail_url" key="empty" class="dropzone__empty">
                      <div class="dropzone__icon" :class="{ bouncing: thumbDragging }">📁</div>
              ...
              <button class="modal-btn modal-btn--primary" @click="submitForm" :disabled="submitting">
                <Transition name="btn-content" mode="out-in">
                  <span v-if="submitting" key="l" class="inline-spinner-wrap">
                    <span class="inline-spinner"></span> {{ modal.isEdit ? 'جاري الحفظ...' : 'جاري النشر...' }}
                  </span>
      ...
      <Transition name="modal">
        <div v-if="deleteTarget" class="modal-overlay" @mousedown.self="deleteTarget = null">
          <div class="modal modal--sm">
            <div class="modal__header">
              <h2 class="modal__title">حذف الخدمة</h2>
            ...
            <div class="modal__footer">
              ...
              <button class="modal-btn modal-btn--danger" @click="deleteService" :disabled="submitting">
                <span v-if="submitting" class="inline-spinner"></span> {{ submitting ? 'جاري الحذف...' : 'نعم، احذف' }}
              </button>
            </div>
          </div>
        </div>
...
const errors = ref({ title: '', description: '', dynamic_price: '', delivery_days: '' })
const form = ref({
  title: '', description: '', category_id: null,
  dynamic_price: '', delivery_days: '',
  thumbnail_url: null,
})

function openModal(svc = null) {
  errors.value = { title: '', description: '', dynamic_price: '', delivery_days: '' }
  ...
  if (svc) {
    ...
    form.value = {
      ...
      dynamic_price: svc.dynamic_price,
      delivery_days: svc.delivery_days,
      thumbnail_url: svc.thumbnail_url,
    }
  } else {
    modal.value = { open: true, isEdit: false, serviceId: null }
    form.value = { title: '', description: '', category_id: null, dynamic_price: '', delivery_days: '', thumbnail_url: null }
  }
...
async function submitForm() {
  ...
  try {
    ...
    if (modal.value.isEdit) {
      ...
      const { data } = await api.post('/seller/services', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      showToast(data.message ?? 'تم إرسال الخدمة لمراجعة الإدارة')
    }
    closeModal()
    fetchServices()
  } catch (e) {
...
const displayStatusLabel = svc => svc.is_pending_review ? 'قيد المراجعة' : statusLabel(svc.status)
const categoryEmoji = c => ({ 'تطوير الويب':'💻','تصميم الجرافيك':'🎨','التسويق الرقمي':'📢','كتابة المحتوى':'✍️','الترجمة':'🌐' }[c] ?? '🛠️')
...
.dot--pending { background: var(--color-primary); }
...
.pending-pill { padding: 6px 14px; border-radius: var(--radius-full); font-size: 11.5px; font-weight: 600; background: rgba(99,102,241,0.12); color: var(--color-primary); }
...
.modal { background: var(--color-bg); border: 1px solid var(--color-border); border-radius: var(--radius-xl); width: 100%; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 24px 80px rgba(0,0,0,0.6), var(--shadow-glow); }
...
/* Field hint */
.field-hint { font-size: 12px; color: var(--color-text-3); font-style: italic; }
