...
class ServiceController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data'  => $services->getCollection()->map(fn($s) => $this->formatService($s)),
            'meta'  => [
    ...
    public function show(int $id): JsonResponse
    {
        ...
        $service->seller->loadCount([
            'requests as completed_orders_count' => fn($q) => $q->where('status', 'completed'),
        ]);
    ...
    public function myServices(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data' => $services->getCollection()->map(fn($s) => $this->formatService($s, detailed: true)),
            'meta' => [
                'current_page' => $services->currentPage(),
                'last_page'    => $services->lastPage(),
                'total'        => $services->total(),
     ...
     *
     * Personalized service suggestions for the logged-in client, based on
     * the categories they picked at signup and their past order history.
     */
    public function recommendations(\App\Services\RecommendationService $recommender): JsonResponse
    {
        $client = Auth::user()->client;
        abort_unless($client, 403, 'Only clients have personalized recommendations.');

        $services = $recommender->forClient($client);

        return response()->json([
            'data' => $services->map(fn($s) => $this->formatService($s))->values(),
        ]);
    }
    ...
    private function formatService(Service $s, bool $detailed = false): array
    {
        $data = [
            ...
            'seller'             => $s->relationLoaded('seller') ? [
                ...
                'bio'            => $detailed ? $s->seller->bio : null,
                'completed_orders_count' => $detailed ? ($s->seller->completed_orders_count ?? 0) : null,
            ] : null,
